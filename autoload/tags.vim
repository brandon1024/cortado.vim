	...
	for tag in l:tags
		...
		if tag.kind == 'c' || tag.kind == 'i' || tag.kind == 'g' || tag.kind == 'a'
			if has_key(tag, 'class')
				let l:fq_name = util#Flatten([l:package, tag.class, tag.name])
			else
				let l:fq_name = util#Flatten([l:package, tag.name])
			endif
		...
		elseif tag.kind == 'm'
			...
			call add(l:prompt_results, {
				...
				\ 'type': tag.kind,
				\ 's': v:true,
				\ 'fq_name': util#Flatten([l:package, tag.class, tag.name])
				\ })
		elseif tag.kind == 'e'
			if !has_key(tag, 'enum')
				continue
			...
			call add(l:prompt_results, {
				...
				\ 'type': tag.kind,
				\ 's': v:true,
				\ 'fq_name': util#Flatten([l:package, tag.enum, tag.name])
				\ })
		endif
	endfor
