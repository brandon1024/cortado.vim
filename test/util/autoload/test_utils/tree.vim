...
function! test_utils#tree#has_node(tree, path) abort
	return len(test_utils#tree#meta_for_node(a:tree, a:path)) ? v:true : v:false
endfunction

" Get the metadata associated with a node in `tree` at path `path`.
" Return a tuple [<leaf node name>, <leaf metadata>]
function! test_utils#tree#meta_for_node(tree, path) abort
